...
description: Preguntas frecuentes sobre CodePushGo
sidebar:
  order: 1
locale: es
---
...
Usamos el término "release" para referirnos a la preparación de un binario para las tiendas de aplicaciones. Para poder generar un bundle posteriormente, CodePushGo necesita conocer el binario exacto que se envió a las tiendas.
...
### ¿Puedo usar CodePushGo con mi equipo?
...
No. Los servidores de CodePushGo nunca ven tu código fuente. Cuando ejecutas `npx @capgo/cli app update`, la herramienta `npx @capgo/cli` solo sube el mismo código compilado que envías a las tiendas de aplicaciones. Si deseas seguridad adicional, puedes usar cifrado de Extremo a Extremo para cifrar tu bundle antes de subirlo a los servidores de CodePushGo.

Consulta también nuestra política de privacidad: [https://capgo.app/privacy](https://capgo.app/privacy/)

### ¿Puedo usar CodePushGo desde mi sistema CI?

Sí. CodePushGo está diseñado para ser usado desde sistemas CI. Hemos publicado una guía para [Android y Github Actions](https://capgo.app/blog/automatic-capacitor-android-build-github-action/) y [IOS](https://capgo.app/blog/automatic-capacitor-ios-build-github-action/), y para [Gitlab](https://capgo.app/blog/setup-ci-and-cd-gitlab/), otros sistemas CI deberían ser similares.
...
Sí. No hay límite en el tamaño de la aplicación que puede actualizarse con code push. Como se menciona [abajo](https://capgo.app/docs/faq/#what-types-of-changes-does-capgo-code-push-support), CodePushGo puede cambiar cualquier código JS en tu aplicación sin importar el tamaño.
...
### ¿Para qué puedo usar CodePushGo code push?
...
### ¿Qué cuenta como un "MAU" para CodePushGo?
...
### ¿Para qué no podemos usar CodePushGo code push?

Como se mencionó arriba, CodePushGo no debe usarse para violar las políticas de las tiendas de aplicaciones. Consulta [abajo](https://capgo.app/docs/faq/#does-capgo-comply-with-play-store-guidelines) para más información.

Además, CodePushGo no soporta cambiar código nativo (por ejemplo, Java/Kotlin en Android u Objective-C/Swift en iOS). La herramienta te advertirá durante un intento de actualización si has cambiado código nativo.

### ¿Capgo envía a las tiendas por mí?

Actualmente CodePushGo no soporta el envío a las tiendas de aplicaciones en tu nombre. Tenemos planes de agregar esto en el futuro, pero por ahora necesitarás continuar usando tus procesos existentes para enviar a las tiendas de aplicaciones.
...
### ¿Qué almacena CodePushGo en el disco y dónde?

El actualizador de CodePushGo (incluido en tu aplicación cuando la compilas) almacena en caché el último bundle descargado en el único directorio que Capacitor permite cargar código. En Android, esto se encuentra en `/data/user/0/comexampleapp/code_cache/capgo_updater` aunque la base de esa ruta es proporcionada por el sistema Android y puede cambiar dinámicamente en tiempo de ejecución. En dispositivos iOS, los datos se almacenan en `Library/Application Support/capgo`.

Las herramientas de línea de comandos de CodePushGo (por ejemplo`npx @capgo/cli app update`) se instalan en el disco en las cachés de npm, tus inicios de sesión se almacenan en el directorio home en `~/capgo`
...
### ¿Qué tipos de cambios soporta el code push de CodePushGo?
...
### ¿Qué versiones de SO soporta CodePushGo?
...
### ¿Qué versiones de Capacitor soporta CodePushGo?
...
### ¿Cumple CodePushGo con las directrices de Play Store?
...
2. Los cambios en la aplicación no deben ser engañosos (por ejemplo, cambiar el propósito de la aplicación mediante actualización) [https://support.google.com/googleplay/android-developer/answer/9888077](https://support.google.com/googleplay/android-developer/answer/9888077/) Por favor, sé claro con tus usuarios sobre lo que estás proporcionando con tu aplicación y no violes sus expectativas con cambios significativos de comportamiento a través del uso de CodePushGo.

Capgo está diseñado para ser compatible con las directrices de Play Store. Sin embargo, CodePushGo es una herramienta y, como cualquier herramienta, puede ser mal utilizada. El abuso deliberado de CodePushGo para violar las directrices de Play Store está en violación de los [Términos de Servicio](https://capgo.app/tos/) de CodePushGo y puede resultar en la terminación de tu cuenta.
...
### ¿Cumple CodePushGo con las directrices de App Store?
...
Capgo utiliza un intérprete personalizado de Dart para cumplir con la restricción de solo intérprete para actualizaciones en iOS. Mientras tu aplicación no participe en comportamientos engañosos a través de actualizaciones (por ejemplo, cambiar el propósito de la aplicación mediante actualización), actualizar a través de CodePushGo (o cualquier otra solución de código push) es una práctica estándar de la industria y cumple con las pautas de la App Store.

El abuso deliberado de CodePushGo para violar las pautas de la App Store está en violación de los [Términos de Servicio](https://capgo.app/tos/) de CodePushGo y puede resultar en la terminación de tu cuenta.

Microsoft también publica una guía sobre cómo su biblioteca "codepush" de react native cumple con las tiendas de aplicaciones: [https://github.com/microsoft/react-native-code-push#store-guideline-compliance](https://github.com/microsoft/react-native-code-push/#store-guideline-compliance)

### ¿Puedo usar CodePushGo en mi país?[](https://capgo.app/docs/faq/#can-i-use-capgo-in-my-country "Direct link to Can I use CodePushGo in my country?")

No hemos intentado restringir el acceso a CodePushGo desde ningún país.

Reconocemos que algunos países tienen restricciones sobre qué URLs se pueden acceder desde dentro del país. CodePushGo actualmente utiliza Cloudflare Cloud para el alojamiento, incluyendo R2 Storage y Cloudflare workers.

Las siguientes URLs son utilizadas por CodePushGo:

-   [https://apicapgo.app](https://apicapgo.app/) -- utilizado por las herramientas de línea de comandos `npx @capgo/cli` para interactuar con los servidores de CodePushGo, así como el actualizador de CodePushGo en los dispositivos de los usuarios para buscar actualizaciones
...
Si todas esas URLs son accesibles desde tu país, entonces CodePushGo debería funcionar.
...
### ¿Puedo auto-alojar CodePushGo?[](https://capgo.app/docs/faq/#can-i-self-host-capgo "Direct link to Can I self-host CodePushGo?")

Sí, puedes auto-alojar CodePushGo. La guía aún no está escrita, pero el código es de código abierto y está disponible en [https://github.com/cap-go/capgo](https://github.com/cap-go/capgo/)
...
### ¿Cómo afecta la falta de conectividad de red a CodePushGo?[](https://capgo.app/docs/faq/#how-is-capgo-affected-by-lack-of-network-connectivity "Direct link to How is CodePushGo affected by lack of network connectivity?")

El actualizador de CodePushGo (incluido en tu aplicación cuando construyes tu app con CodePushGo) está diseñado para ser resiliente a problemas de conectividad de red.

En el comportamiento de actualización predeterminado, cuando la aplicación se inicia, alerta al actualizador de CodePushGo, que genera un hilo separado para hacer una solicitud de red a los servidores de CodePushGo y solicitar una actualización. Intencionalmente usamos un hilo separado para evitar afectar o bloquear cualquier otra cosa que la aplicación pudiera estar haciendo. Si la solicitud de red falla o se agota el tiempo de espera, el actualizador simplemente intentará verificar nuevamente la próxima vez que se inicie la aplicación.

Las herramientas de línea de comandos de CodePushGo (por ejemplo, `npx @capgo/cli app update`) requieren conectividad de red para funcionar. Si estás usando CodePushGo para distribuir tu aplicación, debes asegurarte de que tu sistema CI tenga conectividad de red.
...
### ¿Cómo se relaciona CodePushGo con Capacitor?[](https://capgo.app/docs/faq/#how-does-capgo-relate-to-capacitor "Direct link to How does CodePushGo relate to Capacitor?")

Capgo es un plugin para Capacitor que agrega code push. CodePushGo no es un reemplazo para Capacitor. Puedes continuar usando las herramientas de Capacitor que ya conoces y te gustan.
...
Por defecto, el actualizador de CodePushGo busca actualizaciones al iniciar la aplicación. Se ejecuta en un hilo en segundo plano y no bloquea el hilo de la UI. Cualquier actualización se instalará mientras el usuario está usando la aplicación y se aplicará la próxima vez que se reinicie la aplicación.

También es posible ejecutar el actualizador de CodePushGo manualmente usando [package:capgo\_code\_push](https://pubdev/packages/capgo_code_push/), a través del cual es posible activar actualizaciones en cualquier momento, incluso mediante una notificación push.

El actualizador de CodePushGo está diseñado de tal manera que cuando la red no está disponible, o el servidor está caído o no es accesible, la aplicación continuará funcionando normalmente. Si alguna vez decides eliminar una actualización de nuestros servidores, todos tus clientes continuarán funcionando normalmente.
...
Alguien que tenga tu `app_id` puede obtener la última versión de tu aplicación de los servidores de CodePushGo, pero no puede enviar actualizaciones a tu aplicación ni acceder a ningún otro aspecto de tu cuenta de CodePushGo.

### ¿Qué información se envía a los servidores de CodePushGo?[](https://capgo.app/docs/faq/#what-information-is-sent-to-capgo-servers "Direct link to What information is sent to CodePushGo servers?")

Aunque CodePushGo se conecta a la red, no envía ninguna información personalmente identificable. Incluir CodePushGo no debería afectar tus declaraciones para la Play Store o App Store.

Las solicitudes enviadas desde la aplicación a los servidores de CodePushGo incluyen:
...
### ¿Qué plataformas soporta CodePushGo?

Actualmente, CodePushGo soporta iOS y Android. Ambos están listos para producción.

El uso de CodePushGo para iOS o Android puede ser decisiones independientes. Puedes configurar en tu canal para distribuir a Android y una ipa construida para la App Store o viceversa.
...
### ¿Cómo interactúa CodePushGo con Play Testing Tracks o Apple TestFlight?
...
Desafortunadamente, no todos estos mecanismos permiten a terceros detectar cuándo las aplicaciones están instaladas en alguna Test Track específica o vía TestFlight. Por lo tanto, no tenemos visibilidad confiable sobre la composición de estos grupos, y no podemos controlar de manera confiable el acceso a los parches de CodePushGo basados en estos grupos [https://stackoverflow.com/questions/53291007/can-an-android-application-identify-the-test-track-within-google-play](https://stackoverflow.com/questions/53291007/can-an-android-application-identify-the-test-track-within-google-play/) [https://stackoverflow.com/questions/26081543/how-to-tell-at-runtime-whether-an-ios-app-is-running-through-a-testflight-beta-i](https://stackoverflow.com/questions/26081543/how-to-tell-at-runtime-whether-an-ios-app-is-running-through-a-testflight-beta-i/)

Si deseas segmentar la disponibilidad del paquete CodePushGo, hay 4 opciones potenciales:
...
3. CodePushGo permite crear su propio mecanismo de inscripción por dispositivo (similar a Test Tracks o TestFlight, solo que agnóstico a la plataforma). Esto permite que tu equipo de QA opte por recibir paquetes antes de que sean promovidos al público general.
4. CodePushGo tiene despliegues basados en porcentajes. Esto no te permite elegir a qué dispositivos enviar, pero puede ayudarte a desplegar incrementalmente y revertir ante cualquier problema.
...
Los períodos de facturación se reinician automáticamente cada mes en el mes que te suscribiste por primera vez a CodePushGo. Por ejemplo, si te suscribiste el día 15 del mes, tu período de facturación se reiniciará el 15 de cada mes.
...
MAU es una mejor solución que el número de instalaciones para fijar el precio de CodePushGo, ya que es más preciso y refleja el costo real de CodePushGo por dispositivo.

Por razones de privacidad, no podemos rastrear el mismo dispositivo si el usuario reinstala la aplicación.

Las reglas de privacidad son impuestas por Apple y Google, y no por CodePushGo.
