...
El sistema de Actualización en Vivo de CodePushGo puede entregar actualizaciones más rápido y de manera más eficiente enviando solo los archivos modificados, en lugar del paquete JS completo
...
Las actualizaciones diferenciales en CodePushGo son manejadas por el plugin de CodePushGo instalado en tu aplicación. Cuando subes una nueva versión de tu aplicación usando la bandera `--partial`, CodePushGo hace lo siguiente:
...
4. Este manifiesto se sube a la base de datos de CodePushGo

Cuando un dispositivo ejecutando tu aplicación busca actualizaciones, el plugin de CodePushGo recibe el nuevo manifiesto del servidor. Compara este manifiesto con el que tiene actualmente, identificando qué archivos han cambiado basándose en los checksums y rutas de archivos
...
En caso de actualizaciones diferenciales, el dispositivo almacena todos los archivos descargados en un caché común, CodePushGo nunca lo limpiará pero el sistema operativo puede hacerlo en cualquier momento

</Aside>

## Habilitando Actualizaciones Diferenciales

Para habilitar actualizaciones diferenciales para tu aplicación CodePushGo, simplemente usa la bandera `--partial` cuando subas una nueva versión:
...
Cuando se usa `--partial-only`, CodePushGo solo subirá archivos individuales y generará un manifiesto. Cualquier dispositivo que no soporte parcial no podrá descargar la actualización
...
- Tu dispositivo esté ejecutando la última versión del plugin de CodePushGo
- Tu dispositivo tenga una conexión de red estable y pueda alcanzar los servidores de CodePushGo

También puedes usar la aplicación web de CodePushGo para verificar los detalles de tu última subida:
<Steps>
...
Si continúas teniendo problemas, por favor contacta al soporte de CodePushGo para más asistencia. Ellos pueden revisar los registros del servidor para confirmar que tus subidas parciales están siendo procesadas correctamente y que los dispositivos están recibiendo los manifiestos actualizados

¡Eso es todo! La bandera `--partial` le dice a CodePushGo que realice las subidas de archivos individuales y la generación de manifiestos necesarios para las actualizaciones diferenciales

Ten en cuenta que necesitas usar `--partial` cada vez que subas una nueva versión que quieras que se entregue como una actualización diferencial. Si omites la bandera, CodePushGo subirá el paquete JS completo como un solo archivo, y los dispositivos descargarán el paquete completo incluso si solo una pequeña parte ha cambiado
