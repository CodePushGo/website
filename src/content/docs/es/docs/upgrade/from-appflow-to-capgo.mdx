---
title: Migrar de AppFlow a CodePushGo
description: Guía completa para migrar tu aplicación de Ionic AppFlow a CodePushGo
sidebar:
  order: 7
locale: es
---
...
Esta configuración te ayudará a mapear las características de AppFlow a sus equivalentes en CodePushGo.

## ¿Por qué migrar a CodePushGo?

Con el anuncio del cierre de Ionic AppFlow, migrar a CodePushGo ofrece una transición perfecta para tu flujo de trabajo de desarrollo de aplicaciones móviles. CodePushGo ofrece características mejoradas, mejor rendimiento y ahorros significativos en costos mientras mantiene toda la funcionalidad crítica que necesitas.
...
Agrega la configuración de CodePushGo a tu `capacitor.config.json`:
```json
{
  "plugins": {
    "CapacitorUpdater": {
...
1. Crear canales en el dashboard de CodePushGo:
```bash
...
1. [Crear una cuenta CodePushGo](https://capgo.app/register/)
