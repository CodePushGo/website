---
title: Comandos
description: Documentación CLI de CodePushGo
sidebar:
  order: 2
locale: es
---
...
Comando para verificar si estás actualizado con los paquetes de CodePushGo
...
* `--icon [/ruta/a/mi/icono]` para tener un icono personalizado mostrado en la aplicación web de CodePushGo
...
* `--external <url>` Enlace a URL externa en lugar de subir a CodePushGo Cloud
...
* `--partial` No sube archivos parciales a CodePushGo cloud
...
> ⭐️ La opción externa ayuda a desbloquear 2 casos: corporativos con preocupaciones de privacidad, no enviar el código a terceros y aplicaciones mayores a 200 MB Con esta configuración, CodePushGo solo almacena el enlace al zip y envía el enlace a todas las aplicaciones

> 👀 CodePushGo cloud nunca mira lo que hay en el enlace (para la opción externa), o en el código cuando está almacenado

> 🔑 Puedes agregar una segunda capa de seguridad usando cifrado, entonces CodePushGo no podrá ver ni modificar nada, se vuelve "sin confianza"
