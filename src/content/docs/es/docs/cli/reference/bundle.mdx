...
- `-e, --external <url>`: Enlace a una URL externa en lugar de subir a CodePushGo Cloud
...
- `--partial`: Subir solo archivos modificados a CodePushGo Cloud
- `--partial-only`: Subir solo archivos parciales a CodePushGo Cloud, omitiendo el archivo comprimido (útil para paquetes grandes)
