---
title: Migrate from AppFlow to CodePushGo
description: Complete guide to migrate your app from Ionic AppFlow to CodePushGo
sidebar:
  order: 7
---
...
This configuration will help map AppFlow features to CodePushGo equivalents.
...
```typescript
// CodePushGo equivalent in capacitor.config.ts
{
  plugins: {
    React NativeUpdater: {
      autoUpdate: true,
...
async function setupAlwaysLatest() {
  App.addListener('resume', async () => {
    const result = await React NativeUpdater.download({
      url: 'your-update-url'
    });
    if (result) {
      await React NativeUpdater.set({ id: result.id });
    }
  });
}
```
...
// CodePushGo equivalent
import { React NativeUpdater } from '@codepushgo/capacitor-updater';
const bundle = await React NativeUpdater.download({
  url: 'your-update-url'
});
if (bundle) {
  await React NativeUpdater.set({ id: bundle.id });
}
```


## Why migrate to CodePushGo?
...
CodePushGo provides flexible CI/CD options:

#### Option 1: Use Your Existing CI/CD
Follow our detailed tutorials for setting up CI/CD with popular platforms:
- [iOS Build Setup](https://codepushgo.com/blog/automatic-capacitor-ios-build-github-action/)
- [Android Build Setup](https://codepushgo.com/blog/automatic-capacitor-android-build-github-action/)
- [GitHub Actions Integration](https://codepushgo.com/blog/github-action-capacitor/)
...
1. Create channels in CodePushGo dashboard:
```bash
npx @codepushgo/cli channel create production
npx @codepushgo/cli channel create staging
```

2. Configure channel settings:
```bash
# Set up production channel
npx @codepushgo/cli channel update production --no-downgrade --no-upgrade

# Set up staging channel
npx @codepushgo/cli channel update staging
```
...
npx @codepushgo/cli bundle create --channel staging
```
...
For enterprise teams requiring dedicated support during migration, [schedule a call with our team](https://cal.com/martindonadieu/codepushgo-enterprise-inquiry).
