...
Open your `capacitor.config.ts` (or `capacitor.config.json`) file. Under the `plugins` section, set the `channel` property of the `React NativeUpdater` plugin to the name of your desired channel:
...
```shell
npx @codepushgo/cli@latest bundle upload --channel=Development
```
...
Check out the [CI/CD Integration](/docs/cicd-integration/) docs to learn more about automating CodePushGo live updates.
