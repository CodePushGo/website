---
title: Channels
description: "Learn how to manage and configure Live Update channels in CodePushGo, enabling seamless app updates by directing specific JS bundle builds to devices configured for those channels."
sidebar:
  order: 2
---
...
A Live Update channel points to a specific JS bundle build of your app that will be shared with any devices configured to listen to that channel for updates. When you [install the CodePushGo Live Updates SDK](/docs/getting-started/quickstart/) in your app, any native binary configured to that channel will check for available updates whenever the app is launched. You can change the build a channel points to at any time and can also roll back to previous builds if needed.

## Setting up a Channel

Every app comes with a default channel called "Production" that cannot be deleted. To add new channels:

1. Go to the "Channels" section of the CodePushGo dashboard 
...
To deploy a live update, you need to upload a new JS bundle build and assign it to a channel. You can do this in one step with the CodePushGo CLI:
...
You can also assign builds to channels from the "Bundles" section of the CodePushGo dashboard. Click the menu icon next to a build and select "Assign to Channel" to choose the channel for that build.

## Bundle Versioning and Channels

It's important to note that bundles in CodePushGo are global to your app, not specific to individual channels. The same bundle can be assigned to multiple channels.
...
For more advanced workflows, you can automate your live update deployments as part of your CI/CD pipeline. By integrating CodePushGo into your build process, you can automatically upload new bundles and assign them to channels whenever you push to certain branches or create new releases.

Check out the [CI/CD Integration](/docs/getting-started/cicd-integration/) docs to learn more about automating CodePushGo live updates.

## Deploying to a Device

Now that you understand channels, you're ready to start deploying live updates to real devices. The basic process is:

1. Install the CodePushGo SDK in your app
...
Learn how to implement these advanced use cases in our guide: [How to Segment Users by Plan and Channels for Feature Flags and A/B Testing](https://capgo.app/blog/how-to-segment-users-by-plan-and-channels/).
