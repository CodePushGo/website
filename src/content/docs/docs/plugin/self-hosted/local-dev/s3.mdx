...
description: "How to set up minio to work with codepushgo"
sidebar:
    order: 2
---
...
This tutorial will show how to set up [minio](https://min.io/) to work with codepushgo.

This is not technically required to run codepushgo.
...
Now that we have a S3 server running we need to set up codepushgo edge functions to use our S3 server.

To do that we need to create an ENV file in `codepushgo/supabase` named `.env.local`
