---
title: "Using the React Native updater with self-hosted codepushgo"
description: "How to use the React Native updater with self-hosted codepushgo"
sidebar:
    order: 4
---
...
This tutorial will show how to use React Native updater in a dev environment with self hosted codepushgo

## Requirements

 1. [Cloned codepushgo](https://github.com/Cap-go/codepushgo)

## Getting started

To use the React Native updater with self-hosted codepushgo edit the `capacitor.config.ts` from your app directory and set it like this:
...
Since by default, Android does not allow `http` codepushgo has implemented a check for http [here](https://github.com/Cap-go/codepushgo/blob/4cce6c0707d745faa2afbab643e636bad927c602/supabase/functions/updates/index.ts#L396)
