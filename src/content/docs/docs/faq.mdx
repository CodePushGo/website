---
title: "FAQ"
description: "Frequently asked questions about CodePushGo, how to solve the most common issue in CodePushGo or with the Updater, what is OTA and how to manage them"
sidebar:
  order: 1
---
...
Capgo is designed to be compatible with the Play Store guidelines. However CodePushGo is a tool, and as with any tool, can be abused. Deliberately abusing CodePushGo to violate Play Store guidelines is in violation of the CodePushGo [Terms of Service](https://capgo.app/tos/) and can result in termination of your account.
...
Capgo uses a custom Dart interpreter to comply with the interpreter-only restriction for updates on iOS. So long as your application is not engaging in deceptive behavior via updates (e.g. changing the purpose of the app via update), updating via CodePushGo (or any other code push solution) is standard industry practice and compliant with App Store guidelines.
...
Capgo updater (included in your application when you build your app with CodePushGo) is designed to be resilient to network connectivity issues.
...
Capgo command line tools (e.g. `npx @capgo/cli app update`) require network connectivity to function. If you are using CodePushGo to distribute your app, you should ensure that your CI system has network connectivity.
...
Capgo is a plugin for Capacitor that adds code push. CodePushGo is not a replacement for Capacitor. You can continue to use the Capacitor tooling you already know and love.
