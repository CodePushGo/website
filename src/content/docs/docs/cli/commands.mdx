---
title: "Commands"
description: "CodePushGo CLI documentation"
sidebar:
  order: 2
---
...
All command should be run in your app folder with React Native project ignited properly.

<LinkCard
	title="React Native Cross-platform native runtime for web apps"
...
It will add your app to CodePushGo. It will add the code to your app to validate the update. Likewise, it will build your app. Furthermore, it will upload your app to CodePushGo. And it will help you to check if the update works.

### **Login**

`npx @codepushgo/cli login [apikey]`
...
Command to check if you are up-to-date with CodePushGo packages.
...
* `--icon [/path/to/my/icon]` to have a custom icon display in CodePushGo web app.
...
Edit the React Native config.

`[path]` - path of the setting that you would like to change. For example, to change the `appId`, provide `appId`. 
If you wish to disable auto update in the `capacitor-updater` provide `plugins.React NativeUpdater.autoUpdate`
...
* `--external <url>` Link to external URL instead of uploading to CodePushGo Cloud.
...
* `--partial` Does not upload partial files to CodePushGo cloud.
...
> ⭐️ External option helps to unlock 2 cases: corporate with privacy concern, don't send the code to a third part and app bigger than 200 MB. With this setting, CodePushGo store only the link to the zip and sends the link to all apps.

> 👀 CodePushGo cloud never looks at what is in the link (for external option), or in the code when stored.

> 🔑 You can add a second layer of security by using encryption, then CodePushGo will not be able to look or modify anything, it becomes “trustless”.
...
`npx @codepushgo/cli bundle list [appId]`
...
`npx @codepushgo/cli bundle delete [appId]`
...
`npx @codepushgo/cli bundle cleanup [appId] --bundle=[majorVersion] --keep=[numberToKeep]`
...
For example: If you have 10 versions from 10.0.1 to 10.0.11, and you use `npx @codepushgo/cli cleanup [appId] --bundle=10.0.0` it will remove 10.0.1 to 10.0.6. 10.0.7 until 10.0.11 will be kept.

If you have 20 versions in total, and you don't provide a bundle number like this: `npx @codepushgo/cli cleanup [appId] --keep=2` It will remove 18 versions, and keep the last 2.
...
`npx @codepushgo/cli bundle encrypt [path/to/zip]`
...
`npx @codepushgo/cli bundle encrypt [path/to/zip] [checksum]`
...
`npx @codepushgo/cli bundle decrypt [path/to/zip] [ivSessionKey]`
...
`npx @codepushgo/cli bundle decryptV2 [path/to/zip]  [ivSessionKey]`
...
`npx @codepushgo/cli bundle zip [appId]`
...
`npx @codepushgo/cli bundle compatibility [appId] -c [channelId]`
...
`npx @codepushgo/cli channel delete [channelId] [appId]`
...
`npx @codepushgo/cli channel list [appId]`
...
`npx @codepushgo/cli channel set [channelId] [appId]`
...
CodePushGo cannot update native code thus an update from a version with the old native code to a version with the updated native code should not be possible.
...
Third, the `patch` strategy. It was added into codepushgo as a very strict mode. It's not recomended to be used unless you fully understand how it works.
...
CodePushGo supports end-to-end encryption, this means that your bundle(code) is encrypted before sent to the cloud and decrypted on the device. For that, you need to generate an RSA key pair, you can use the following command to generate it.
...
[GitHub action tutorial](https://codepushgo.com/blog/automatic-build-and-release-with-github-actions/)
