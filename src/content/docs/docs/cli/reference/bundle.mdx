---
title: 📦 bundle
description: 📦 Manage app bundles for deployment in CodePushGo Cloud, including upload, compatibility checks, and encryption.
sidebar_label: bundle
sidebar:
  order: 4
---

📦 Manage app bundles for deployment in CodePushGo Cloud, including upload, compatibility checks, and encryption.
...
⬆️ Upload a new app bundle to CodePushGo Cloud for distribution, optionally linking to a channel or external URL.
...
> ℹ️ CodePushGo Cloud never looks at the content in the link for external options or in the code when stored.

> ℹ️ You can add a second layer of security with encryption, making CodePushGo trustless.
...
| **-e,** | <code>string</code> | Link to external URL instead of upload to CodePushGo Cloud |
...
| **--zip** | <code>boolean</code> | Upload the bundle using zip to CodePushGo cloud (legacy) |
| **--tus** | <code>boolean</code> | Upload the bundle using TUS to CodePushGo cloud |
| **--tus-chunk-size** | <code>string</code> | Chunk size for the TUS upload |
| **--partial** | <code>boolean</code> | Upload partial files to CodePushGo cloud |
| **--partial-only** | <code>boolean</code> | Upload only partial files to CodePushGo cloud, skip the zipped file, useful for big bundle |
...
| **--dry-upload** | <code>boolean</code> | Dry upload the bundle process, mean it will not upload the files but add the row in database (Used by CodePushGo for internal testing) |
...
| **--supa-host** | <code>string</code> | Supabase host URL, for self-hosted CodePushGo or testing |
| **--supa-anon** | <code>string</code> | Supabase anon token, for self-hosted CodePushGo or testing |
...
🧪 Check compatibility of a bundle with a specific channel in CodePushGo Cloud to ensure updates are safe.
...
🗑️ Delete a specific bundle from CodePushGo Cloud, optionally targeting a single version.
...
📋 List all bundles uploaded for an app in CodePushGo Cloud.
...
🧹 Cleanup old bundles in CodePushGo Cloud, keeping a specified number of recent versions or those linked to channels.
