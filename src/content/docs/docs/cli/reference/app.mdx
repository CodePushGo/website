---
title: 📱 app
description: 📱 Manage your CodePushGo app settings and configurations in CodePushGo Cloud.
sidebar_label: app
sidebar:
  order: 5
---

📱 Manage your CodePushGo app settings and configurations in CodePushGo Cloud.
...
➕ Add a new app to CodePushGo Cloud with a unique app ID in the format com.test.app.
...
| -------------- | ------------- | -------------------- |
| **-n,** | <code>string</code> | App name for display in CodePushGo Cloud |
| **-i,** | <code>string</code> | App icon path for display in CodePushGo Cloud |
...
🗑️ Delete an app from CodePushGo Cloud, optionally specifying a version to delete only that bundle.
...
📋 List all apps registered under your account in CodePushGo Cloud.
...
🐞 Listen for live update events in CodePushGo Cloud to debug your app.
...
⚙️ Update settings for an existing app in CodePushGo Cloud, such as name, icon, or retention period for bundles.
