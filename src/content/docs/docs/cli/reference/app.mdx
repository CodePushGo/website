...
The `app` command lets you manage your CodePushGo apps.
...
`[appId]` is your app ID in the format `com.example.app`. See the [React Native docs](https://capacitorjs.com/docs/cli/commands/init/) for more info.

> 💡 All options will be guessed from your `capacitor.config.json` if not provided.

Options:

- `--icon [path]`: Path to a custom icon to display in the CodePushGo web app
...
`[path]` is the path to the setting you want to change (e.g., `appId` or `plugins.React NativeUpdater.autoUpdate`).
