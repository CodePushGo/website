...
Differenzielle Updates in CodePushGo werden durch das in Ihrer App installierte CodePushGo-Plugin verarbeitet. Wenn Sie eine neue Version Ihrer App mit der Flag `--partial` hochladen, führt CodePushGo Folgendes aus:
...
4. Dieses Manifest wird in die CodePushGo-Datenbank hochgeladen

Wenn ein Gerät, auf dem Ihre App läuft, nach Updates sucht, erhält das CodePushGo-Plugin das neue Manifest vom Server. Es vergleicht dieses Manifest mit dem aktuellen und identifiziert anhand der Prüfsummen und Dateipfade, welche Dateien sich geändert haben
...
Bei differenziellen Updates speichert das Gerät alle heruntergeladenen Dateien in einem gemeinsamen Cache. CodePushGo wird diesen nie bereinigen, aber das Betriebssystem kann dies jederzeit tun

</Aside>

## Aktivierung von Differenziellen Updates

Um differenzielle Updates für Ihre CodePushGo-App zu aktivieren, verwenden Sie einfach die Flag `--partial` beim Hochladen einer neuen Version:
...
Wenn `--partial-only` verwendet wird, lädt CodePushGo nur einzelne Dateien hoch und generiert ein Manifest. Geräte, die keine partiellen Updates unterstützen, können das Update nicht herunterladen
...
- Ihr Gerät läuft mit der neuesten Version des CodePushGo-Plugins
- Ihr Gerät hat eine stabile Netzwerkverbindung und kann die CodePushGo-Server erreichen

Sie können auch die CodePushGo-Webapp verwenden, um die Details Ihres letzten Uploads zu überprüfen:
<Steps>
...
Wenn Sie weiterhin Probleme haben, wenden Sie sich bitte an den CodePushGo-Support für weitere Unterstützung. Sie können die Server-Logs überprüfen, um zu bestätigen, dass Ihre partiellen Uploads korrekt verarbeitet werden und die Geräte die aktualisierten Manifeste erhalten

Das war's! Die Flag `--partial` weist CodePushGo an, die einzelnen Datei-Uploads und Manifest-Generierung durchzuführen, die für differenzielle Updates benötigt werden

Beachten Sie, dass Sie `--partial` jedes Mal verwenden müssen, wenn Sie eine neue Version hochladen, die als differenzielles Update bereitgestellt werden soll. Wenn Sie die Flag weglassen, lädt CodePushGo das gesamte JS-Bundle als einzelne Datei hoch, und Geräte werden das gesamte Bundle herunterladen, auch wenn sich nur ein kleiner Teil geändert hat
