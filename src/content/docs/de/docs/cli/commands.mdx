---
title: Befehle
description: CodePushGo CLI-Dokumentation
sidebar:
  order: 2
locale: de
---
...
Sie fügt Ihre App zu CodePushGo hinzu Sie fügt den Code zur Validierung des Updates in Ihre App ein Ebenso wird sie Ihre App erstellen Außerdem wird sie Ihre App zu CodePushGo hochladen Und sie hilft Ihnen zu überprüfen, ob das Update funktioniert
...
Befehl zur Überprüfung, ob Sie mit den CodePushGo-Paketen auf dem neuesten Stand sind
...
* `--icon [/path/to/my/icon]` für ein benutzerdefiniertes Icon in der CodePushGo Web-App
...
* `--external <url>` Link zu externer URL anstelle des Uploads zur CodePushGo Cloud
...
* `--partial` Lädt keine partiellen Dateien zur CodePushGo-Cloud hoch
...
> ⭐️ Die externe Option hilft zwei Fälle zu lösen: Unternehmen mit Datenschutzbedenken, die keinen Code an Dritte senden möchten und Apps größer als 200 MB Mit dieser Einstellung speichert CodePushGo nur den Link zur ZIP-Datei und sendet den Link an alle Apps

> 👀 CodePushGo Cloud schaut nie, was sich im Link (für externe Option) oder im Code befindet, wenn gespeichert

> 🔑 Sie können eine zweite Sicherheitsebene durch Verschlüsselung hinzufügen, dann kann CodePushGo nichts einsehen oder modifizieren, es wird "vertrauenslos"
...
Drittens die `patch` Strategie Sie wurde als sehr strenger Modus in CodePushGo eingeführt Es wird nicht empfohlen, sie zu verwenden, es sei denn, Sie verstehen vollständig, wie sie funktioniert
