...
- `-e, --external <url>`: Link zu einer externen URL statt Upload zu CodePushGo Cloud
...
- `--partial`: Nur geänderte Dateien zu CodePushGo Cloud hochladen
- `--partial-only`: Nur partielle Dateien zu CodePushGo Cloud hochladen, ZIP-Datei überspringen (nützlich für große Bundles)
