---
title: Migration von AppFlow zu CodePushGo
description: Vollständiger Leitfaden zur Migration Ihrer App von Ionic AppFlow zu CodePushGo
sidebar:
  order: 7
locale: de
---
...
Diese Konfiguration hilft Ihnen, AppFlow-Funktionen den CodePushGo-Äquivalenten zuzuordnen.

## Warum zu CodePushGo migrieren?

Mit der Ankündigung der Einstellung von Ionic AppFlow bietet die Migration zu CodePushGo einen nahtlosen Übergang für Ihren Mobile-App-Entwicklungsworkflow. CodePushGo bietet verbesserte Funktionen, bessere Leistung und erhebliche Kosteneinsparungen bei Beibehaltung aller wichtigen Funktionalitäten.
...
Fügen Sie die CodePushGo-Konfiguration zu Ihrer `capacitor.config.json` hinzu:
```json
{
  "plugins": {
    "CapacitorUpdater": {
...
1. Kanäle im CodePushGo-Dashboard erstellen:
```bash
