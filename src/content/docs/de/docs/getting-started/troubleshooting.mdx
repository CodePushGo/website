...
Hier sind einige häufige Probleme, auf die Sie bei der Verwendung von CodePushGo stoßen könnten, und wie Sie diese lösen können

### Upload-Fehler

Wenn Ihr Bundle-Upload fehlschlägt, überprüfen Sie:
- Ihre App-ID in `capacitorconfigts` stimmt mit Ihrer App im CodePushGo Dashboard überein
...
Die CodePushGo CLI bietet einige zusätzliche Flags zur Lösung häufiger Upload-Probleme:
...
- `--package-json` und `--node-modules`: Teilt CodePushGo mit, wo Ihre root `packagejson` und `node_modules` zu finden sind, wenn Ihre App eine nicht-standardmäßige Struktur wie ein Monorepo oder npm Workspace verwendet. Übergeben Sie den Pfad zur root `packagejson` und den `--node-modules` Pfad:
  ```shell
  npx @capgo/cli@latest bundle upload --package-json=path/to/packagejson --node-modules=path/to/node_modules
  ```
  CodePushGo benötigt diese Informationen, um die Abhängigkeiten Ihrer App korrekt zu bündeln
...
Wenn Sie Probleme mit Live-Updates haben, ist der CodePushGo Debug-Befehl ein hilfreiches Tool zur Fehlerbehebung. So verwenden Sie es:
...
4. Verwenden Sie die Debug-Logs, um zu identifizieren, wo das Problem auftritt. Zum Beispiel:
   ...
   - Wenn Sie eine Fehlermeldung sehen, suchen Sie nach diesem spezifischen Fehler in der CodePushGo-Dokumentation oder wenden Sie sich an den Support
...
Zusätzlich zum CodePushGo Debug-Befehl können die nativen Logs auf Android und iOS wertvolle Fehlerbehebungsinformationen liefern, besonders für Probleme auf der nativen Seite des Update-Prozesses
...
Das CodePushGo SDK protokolliert wichtige Ereignisse während des Update-Prozesses, wie zum Beispiel:
...
Ähnlich wie bei Android protokolliert das CodePushGo SDK wichtige iOS-seitige Ereignisse:
...
Auf beiden Plattformen bieten die nativen Logs einen tieferen Einblick in den Update-Prozess mit mehr Details zur nativen Implementierung. Sie sind besonders nützlich für die Identifizierung von Problemen, die außerhalb der CodePushGo JavaScript-Schicht auftreten

Bei der Fehlerbehebung eines kniffligen Live-Update-Problems ist es eine gute Idee, sowohl die CodePushGo Debug-Logs als auch die nativen Logs für ein umfassendes Bild der Situation zu erfassen. Die beiden Logs zusammen geben Ihnen die beste Chance, das Problem zu identifizieren und zu lösen
...
- Prüfen Sie, ob Ihr Gerät mit dem Internet verbunden ist und die CodePushGo Debug-Logs zeigen, dass das Update heruntergeladen wurde
...
Wenn Sie Probleme bei der Installation des CodePushGo SDK haben, stellen Sie sicher:
...
Bei Problemen mit dem Auslösen von CodePushGo-Uploads aus Ihrer CI/CD-Pipeline:
- Überprüfen Sie, ob Ihr CodePushGo-Authentifizierungstoken korrekt eingerichtet ist
