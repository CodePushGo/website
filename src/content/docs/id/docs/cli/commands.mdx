---
title: Perintah
description: Dokumentasi CodePushGo CLI
sidebar:
  order: 2
locale: id
---
...
Perintah untuk memeriksa apakah Anda sudah up-to-date dengan paket CodePushGo
...
* `--icon [/path/to/my/icon]` untuk memiliki ikon khusus yang ditampilkan di aplikasi web CodePushGo
...
* `--external <url>` Link ke URL eksternal alih-alih mengunggah ke CodePushGo Cloud
...
* `--partial` Tidak mengunggah file parsial ke cloud CodePushGo
...
> ⭐️ Opsi eksternal membantu membuka 2 kasus: korporat dengan masalah privasi, jangan kirim kode ke pihak ketiga dan aplikasi lebih besar dari 200 MB Dengan pengaturan ini, CodePushGo hanya menyimpan tautan ke zip dan mengirim tautan ke semua aplikasi

> 👀 Cloud CodePushGo tidak pernah melihat apa yang ada di link (untuk opsi eksternal), atau di kode ketika disimpan

> 🔑 Anda dapat menambahkan lapisan keamanan kedua dengan menggunakan enkripsi, maka CodePushGo tidak akan dapat melihat atau memodifikasi apa pun, menjadi "trustless"
