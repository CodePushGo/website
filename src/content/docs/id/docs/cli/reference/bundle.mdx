...
- `-e, --external <url>`: Link ke URL eksternal daripada mengunggah ke CodePushGo Cloud
...
- `--partial`: Unggah hanya file yang berubah ke CodePushGo Cloud
- `--partial-only`: Unggah hanya file parsial ke CodePushGo Cloud, melewati file zip (berguna untuk bundel besar)
