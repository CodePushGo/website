...
Berikut adalah beberapa masalah umum yang mungkin Anda temui saat menggunakan CodePushGo dan cara mengatasinya

### Kegagalan upload

Jika upload bundle Anda gagal, periksa kembali:
- ID aplikasi di `capacitor.config.ts` sesuai dengan aplikasi Anda di dashboard CodePushGo
...
CLI CodePushGo menyediakan beberapa flag tambahan untuk membantu masalah upload yang umum:
...
- `--package-json` dan `--node-modules`: Memberi tahu CodePushGo di mana menemukan root `package.json` dan `node_modules` jika aplikasi Anda menggunakan struktur non-standar seperti monorepo atau workspace npm. Berikan path ke root `package.json` dan path `--node_modules`:
  ```shell
  npx @capgo/cli@latest bundle upload --package-json=path/to/package.json --node_modules=path/to/node_modules
  ```
  CodePushGo membutuhkan informasi ini untuk membundle dependensi aplikasi Anda dengan benar

Anda dapat menggabungkan flag-flag ini dengan opsi lain seperti `--channel` sesuai kebutuhan. Lihat [dokumentasi CLI CodePushGo](/docs/cli/overview/) untuk detail lengkap tentang opsi upload yang tersedia

Jika Anda masih mengalami masalah dengan upload, hubungi [dukungan CodePushGo](mailto:support@capgo.app) untuk bantuan lebih lanjut

### Debug Update

Jika Anda mengalami masalah dengan pembaruan langsung, perintah debug CodePushGo adalah alat yang membantu untuk troubleshooting. Cara menggunakannya:
...
4. Gunakan log debug untuk mengidentifikasi di mana masalah terjadi. Contohnya:
   ...
   - Jika Anda melihat pesan error, cari error tersebut di dokumentasi CodePushGo atau hubungi dukungan untuk bantuan
...
Selain perintah debug CodePushGo, log native di Android dan iOS dapat memberikan informasi troubleshooting yang berharga, terutama untuk masalah di sisi native dari proses pembaruan
...
SDK CodePushGo akan mencatat kejadian-kejadian penting selama proses pembaruan, seperti:
...
Mirip dengan Android, SDK CodePushGo akan mencatat kejadian-kejadian penting di sisi iOS:
...
Pada kedua platform, log native memberikan tampilan proses pembaruan yang lebih detail, dengan rincian lebih lanjut tentang implementasi native. Log ini sangat berguna untuk mengidentifikasi masalah yang terjadi di luar lapisan JavaScript CodePushGo

Saat troubleshooting masalah pembaruan langsung yang rumit, sebaiknya mengambil log debug CodePushGo dan log native untuk gambaran komprehensif tentang apa yang terjadi. Kedua log bersama akan memberi Anda peluang terbaik untuk mengidentifikasi dan menyelesaikan masalah
...
- Periksa bahwa perangkat Anda terhubung ke internet dan log debug CodePushGo menunjukkan pembaruan telah diunduh
...
Jika Anda mengalami masalah menginstal SDK CodePushGo, pastikan:
...
Untuk masalah dengan memicu upload CodePushGo dari pipeline CI/CD Anda:
- Periksa kembali token autentikasi CodePushGo Anda telah diatur dengan benar
