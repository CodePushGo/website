---
title: Migration d'AppFlow vers CodePushGo
description: Guide complet pour migrer votre application d'Ionic AppFlow vers CodePushGo
sidebar:
  order: 7
locale: fr
---
...
Cette configuration vous aidera à mapper les fonctionnalités d'AppFlow vers leurs équivalents CodePushGo.
...
```typescript
// Équivalent CodePushGo dans capacitor.config.ts
{
  plugins: {
    CapacitorUpdater: {
      autoUpdate: true,
...
Si vous utilisez la stratégie "always latest" d'AppFlow, implémentez avec CodePushGo :
...
| Méthode AppFlow | Équivalent CodePushGo | Notes |
|----------------|------------------|-------|
...
## Pourquoi migrer vers CodePushGo ?

Suite à l'annonce concernant Ionic AppFlow, la migration vers CodePushGo offre une transition fluide pour votre processus de développement d'applications mobiles. CodePushGo propose des fonctionnalités améliorées, de meilleures performances et des économies significatives tout en maintenant toutes les fonctionnalités essentielles.
...
Ajoutez la configuration CodePushGo à votre `capacitor.config.json` :
```json
{
  "plugins": {
    "CapacitorUpdater": {
...
1. Créer des canaux dans le tableau de bord CodePushGo :
```bash
...
1. [Créer un compte CodePushGo](https://capgo.app/register/)
