---
title: Commandes
description: Documentation CodePushGo CLI
sidebar:
  order: 2
locale: fr
---
...
Elle ajoutera votre application à CodePushGo Elle ajoutera le code à votre application pour valider la mise à jour De même, elle construira votre application De plus, elle téléversera votre application vers CodePushGo Et elle vous aidera à vérifier si la mise à jour fonctionne
...
Commande pour vérifier si vous êtes à jour avec les paquets CodePushGo
...
* `--icon [/chemin/vers/mon/icone]` pour avoir une icône personnalisée affichée dans l'application web CodePushGo
...
* `--external <url>` Lien vers une URL externe au lieu de téléverser vers CodePushGo Cloud
...
* `--partial` Ne téléverse pas les fichiers partiels vers CodePushGo cloud
...
> ⭐️ L'option externe aide à débloquer 2 cas : entreprise avec des préoccupations de confidentialité, ne pas envoyer le code à un tiers et application plus grande que 200 MB Avec ce paramètre, CodePushGo stocke uniquement le lien vers le zip et envoie le lien à toutes les applications

> 👀 CodePushGo cloud ne regarde jamais ce qu'il y a dans le lien (pour l'option externe), ou dans le code lorsqu'il est stocké

> 🔑 Vous pouvez ajouter une deuxième couche de sécurité en utilisant le chiffrement, alors CodePushGo ne pourra pas regarder ou modifier quoi que ce soit, cela devient "trustless"
