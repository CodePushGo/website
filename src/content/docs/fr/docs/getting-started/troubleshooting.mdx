...
Voici quelques problèmes courants que vous pourriez rencontrer en utilisant CodePushGo et comment les résoudre

### Échecs de téléchargement

Si le téléchargement de votre bundle échoue, vérifiez :
- L'ID de votre application dans `capacitor.config.ts` correspond à votre application dans le tableau de bord CodePushGo
...
La CLI CodePushGo fournit des options supplémentaires pour aider avec les problèmes courants de téléchargement :
...
- `--package-json` et `--node-modules` : Indique à CodePushGo où trouver votre `package.json` racine et `node_modules` si votre application utilise une structure non standard comme un monorepo ou un espace de travail npm. Passez le chemin vers le `package.json` racine et le chemin `--node-modules` :
  ```shell
  npx @capgo/cli@latest bundle upload --package-json=path/to/package.json --node-modules=path/to/node_modules
  ```
  CodePushGo a besoin de ces informations pour regrouper correctement les dépendances de votre application

Vous pouvez combiner ces options avec d'autres comme `--channel` selon vos besoins. Consultez la [documentation CLI CodePushGo](/docs/cli/overview/) pour tous les détails sur les options de téléchargement disponibles

Si vous rencontrez toujours des problèmes avec les téléchargements, contactez le [support CodePushGo](mailto:support@capgo.app) pour obtenir de l'aide

### Débogage des mises à jour

Si vous rencontrez des problèmes avec les mises à jour en direct, la commande debug de CodePushGo est un outil utile pour le dépannage. Pour l'utiliser :
...
4. Utilisez les journaux de débogage pour identifier où se produit le problème. Par exemple :
   ...
   - Si vous voyez un message d'erreur, recherchez cette erreur spécifique dans la documentation CodePushGo ou contactez le support pour obtenir de l'aide
...
En plus de la commande debug de CodePushGo, les journaux natifs sur Android et iOS peuvent fournir des informations de dépannage précieuses, en particulier pour les problèmes côté natif du processus de mise à jour
...
Le SDK CodePushGo enregistrera les événements clés pendant le processus de mise à jour, tels que :
...
Comme pour Android, le SDK CodePushGo enregistrera les événements clés côté iOS :
...
Sur les deux plateformes, les journaux natifs fournissent une vue plus détaillée du processus de mise à jour, avec plus de détails sur l'implémentation native. Ils sont particulièrement utiles pour identifier les problèmes qui se produisent en dehors de la couche JavaScript de CodePushGo

Lors du dépannage d'un problème complexe de mise à jour en direct, il est recommandé de capturer à la fois les journaux de débogage CodePushGo et les journaux natifs pour avoir une vision complète de ce qui se passe. Les deux journaux ensemble vous donneront la meilleure chance d'identifier et de résoudre le problème
...
- Vérifiez que votre appareil est connecté à Internet et que les journaux de débogage CodePushGo montrent que la mise à jour a été téléchargée
...
Si vous rencontrez des problèmes lors de l'installation du SDK CodePushGo, assurez-vous que :
...
Pour les problèmes de déclenchement des téléchargements CodePushGo depuis votre pipeline CI/CD :
- Vérifiez que votre jeton d'authentification CodePushGo est correctement configuré
