...
Utilisez la fonctionnalité Live Updates de CodePushGo pour mettre à jour les bundles JavaScript de votre application à distance, en temps réel. Envoyez les mises à jour JS directement à vos utilisateurs sans passer par le processus de révision de l'app store pour corriger instantanément les bugs et déployer de nouvelles fonctionnalités.
...
Lorsque vous téléchargez un nouveau bundle JS sur CodePushGo et l'assignez à un canal, le SDK CodePushGo dans les applications configurées pour ce canal détectera la mise à jour et la téléchargera. La prochaine fois que l'application redémarre, le nouveau bundle sera chargé.
...
2. Dans votre code d'application, appelez `CapacitorUpdaternotifyAppReady()` après l'initialisation de votre application. Cela indique au SDK CodePushGo que votre application est prête à recevoir des mises à jour
