---
title: AppFlow에서 CodePushGo로 마이그레이션
description: Ionic AppFlow에서 CodePushGo로 앱을 마이그레이션하는 완벽한 가이드
sidebar:
  order: 7
locale: ko
---
...
이 설정은 AppFlow 기능을 CodePushGo의 동등한 기능으로 매핑하는 데 도움이 됩니다.

## CodePushGo로 마이그레이션하는 이유

Ionic AppFlow 서비스 종료 발표와 함께, CodePushGo로의 마이그레이션은 모바일 앱 개발 워크플로우를 원활하게 전환할 수 있는 기회를 제공합니다. CodePushGo는 모든 중요한 기능을 유지하면서 향상된 기능, 더 나은 성능, 그리고 상당한 비용 절감을 제공합니다.
...
#### 설정 업데이트
`capacitor.config.json`에 CodePushGo 설정 추가:
```json
{
  "plugins": {
    "CapacitorUpdater": {
