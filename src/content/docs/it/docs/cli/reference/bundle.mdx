...
- `-e, --external <url>`: Link a un URL esterno invece di caricare su CodePushGo Cloud
...
- `--partial`: Carica solo i file modificati su CodePushGo Cloud
- `--partial-only`: Carica solo file parziali su CodePushGo Cloud, saltando il file zippato (utile per bundle grandi)
