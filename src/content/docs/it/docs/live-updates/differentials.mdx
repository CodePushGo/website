...
Il sistema di Live Update di CodePushGo può distribuire aggiornamenti più velocemente e in modo più efficiente inviando solo i file modificati, invece dell'intero bundle JS
...
Gli aggiornamenti differenziali in CodePushGo sono gestiti dal plugin CodePushGo installato nella tua app. Quando carichi una nuova versione della tua app usando il flag `--partial`, CodePushGo esegue quanto segue:
...
4. Questo manifest viene caricato nel database di CodePushGo

Quando un dispositivo che esegue la tua app verifica la presenza di un aggiornamento, il plugin CodePushGo riceve il nuovo manifest dal server. Confronta questo manifest con quello che ha attualmente, identificando quali file sono cambiati in base ai checksum e ai percorsi dei file
...
In caso di aggiornamenti differenziali, il dispositivo memorizza tutti i file scaricati in una cache comune, CodePushGo non la pulirà mai ma il sistema operativo può farlo in qualsiasi momento

</Aside>

## Abilitare gli Aggiornamenti Differenziali

Per abilitare gli aggiornamenti differenziali per la tua app CodePushGo, usa semplicemente il flag `--partial` quando carichi una nuova versione:
...
Quando viene usato `--partial-only`, CodePushGo caricherà solo file individuali e genererà un manifest. Qualsiasi dispositivo che non supporta il parziale non sarà in grado di scaricare l'aggiornamento
...
- Il tuo dispositivo sta eseguendo l'ultima versione del plugin CodePushGo
- Il tuo dispositivo ha una connessione di rete stabile e può raggiungere i server CodePushGo

Puoi anche usare la webapp CodePushGo per controllare i dettagli del tuo ultimo caricamento:
<Steps>
...
Se continui ad avere problemi, contatta il supporto CodePushGo per ulteriore assistenza. Possono controllare i log del server per confermare che i tuoi caricamenti parziali vengono elaborati correttamente e che i dispositivi stanno ricevendo i manifest aggiornati

Ecco fatto! Il flag `--partial` dice a CodePushGo di eseguire i caricamenti dei file individuali e la generazione del manifest necessari per gli aggiornamenti differenziali

Nota che devi usare `--partial` ogni volta che carichi una nuova versione che vuoi sia distribuita come aggiornamento differenziale. Se ometti il flag, CodePushGo caricherà l'intero bundle JS come un singolo file, e i dispositivi scaricheranno l'intero bundle anche se solo una piccola parte è cambiata
