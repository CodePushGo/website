...
Un canale Live Update punta a una specifica build del bundle JS della tua app che sarà condivisa con qualsiasi dispositivo configurato per ascoltare quel canale per gli aggiornamenti. Quando [installi l'SDK CodePushGo Live Updates](/docs/getting-started/quickstart/) nella tua app, ogni binario nativo configurato su quel canale controllerà gli aggiornamenti disponibili ogni volta che l'app viene avviata. Puoi cambiare la build a cui punta un canale in qualsiasi momento e puoi anche tornare a build precedenti se necessario.
...
1. Vai alla sezione "Channels" della dashboard CodePushGo
...
Per distribuire un aggiornamento live, devi caricare una nuova build del bundle JS e assegnarla a un canale. Puoi farlo in un unico passaggio con la CLI di CodePushGo:
...
Puoi anche assegnare le build ai canali dalla sezione "Bundles" della dashboard CodePushGo. Clicca sull'icona del menu accanto a una build e seleziona "Assign to Channel" per scegliere il canale per quella build.

## Versionamento dei Bundle e Canali

È importante notare che i bundle in CodePushGo sono globali per la tua app, non specifici per i singoli canali. Lo stesso bundle può essere assegnato a più canali.
...
Per flussi di lavoro più avanzati, puoi automatizzare i tuoi deployment di aggiornamenti live come parte della tua pipeline CI/CD. Integrando CodePushGo nel tuo processo di build, puoi caricare automaticamente nuovi bundle e assegnarli ai canali ogni volta che fai push su determinati branch o crei nuove release.

Consulta la documentazione [Integrazione CI/CD](/docs/getting-started/cicd-integration/) per saperne di più sull'automazione degli aggiornamenti live di CodePushGo.
...
1. Installare l'SDK CodePushGo nella tua app
