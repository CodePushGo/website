---
title: FAQ
description: Domande frequenti su CodePushGo
sidebar:
  order: 1
locale: it
---
...
Code push, chiamato anche "aggiornamenti over the air" (OTA), è un servizio cloud che permette agli sviluppatori Capacitor di distribuire aggiornamenti alle loro app in produzione. CodePushGo funziona attualmente su Android e iOS e in futuro funzionerà ovunque funzioni Capacitor.
...
Usiamo il termine "release" per indicare la preparazione di un binario per gli app store. Per poter generare un bundle successivamente, CodePushGo deve conoscere l'esatto binario inviato agli app store.
...
### Posso usare CodePushGo con il mio team?
...
### CodePushGo memorizza il mio codice sorgente?

No. I server CodePushGo non vedono mai il tuo codice sorgente. Quando esegui `npx @capgo/cli app update`, il tool `npx @capgo/cli` carica solo lo stesso codice compilato che invii agli app store. Se desideri ulteriore sicurezza, puoi utilizzare la crittografia End to End per crittografare il tuo bundle prima di caricarlo sui server CodePushGo.

Vedi anche la nostra privacy policy: [https://capgo.app/privacy](https://capgo.app/privacy/)

### Posso usare CodePushGo dal mio sistema CI?

Sì. CodePushGo è pensato per essere usato dai sistemi CI. Abbiamo pubblicato una guida per [Android e Github Actions](https://capgo.app/blog/automatic-capacitor-android-build-github-action/) e [IOS](https://capgo.app/blog/automatic-capacitor-ios-build-github-action/), e per [Gitlab](https://capgo.app/blog/setup-ci-and-cd-gitlab/), altri sistemi CI dovrebbero essere simili.
...
Sì. Non c'è limite alla dimensione dell'applicazione che può essere aggiornata con code push. Come notato [sotto](https://capgo.app/docs/faq/#what-types-of-changes-does-capgo-code-push-support), CodePushGo può modificare qualsiasi codice JS nella tua applicazione indipendentemente dalla dimensione.
...
### Per cosa posso usare il code push di CodePushGo?
...
### Cosa conta come "MAU" per CodePushGo?
...
### Per cosa non possiamo usare il code push di CodePushGo?

Come sopra, CodePushGo non dovrebbe essere usato per violare le policy degli app store. Vedi [sotto](https://capgo.app/docs/faq/#does-capgo-comply-with-play-store-guidelines) per maggiori informazioni.

Inoltre CodePushGo non supporta la modifica del codice nativo (es. Java/Kotlin su Android o Objective-C/Swift su iOS). Lo strumento ti avviserà durante un tentativo di aggiornamento se hai modificato codice nativo.

### CodePushGo invia agli store per me?

Attualmente CodePushGo non supporta l'invio agli app store per tuo conto. Abbiamo in programma di aggiungere questa funzionalità in futuro, ma per ora dovrai continuare a utilizzare i tuoi processi esistenti per inviare agli app store.
...
### Cosa memorizza CodePushGo su disco e dove?

L'updater di CodePushGo (incluso nella tua applicazione quando compili la tua app) memorizza nella cache l'ultimo bundle scaricato nell'unica directory da cui capacitor permette di caricare codice. Su Android, si trova in `/data/user/0/comexampleapp/code_cache/capgo_updater` anche se la base di quel percorso è fornita dal sistema Android e può cambiare dinamicamente durante l'esecuzione. Sui dispositivi iOS, i dati sono memorizzati sotto `Library/Application Support/capgo`.

Gli strumenti da riga di comando di CodePushGo (es.`npx @capgo/cli app update`) vengono installati su disco nelle cache npm, i tuoi login sono memorizzati nella tua directory home in `~/capgo`
...
### Che tipo di modifiche supporta il code push di CodePushGo?[](https://capgo.app/docs/faq/#what-types-of-changes-does-capgo-code-push-support "Link diretto a Che tipo di modifiche supporta il code push di CodePushGo?")
...
### Quali versioni OS supporta CodePushGo?[](https://capgo.app/docs/faq/#what-os-versions-does-capgo-support "Link diretto a Quali versioni OS supporta CodePushGo?")
...
### Quali versioni di Capacitor supporta CodePushGo?[](https://capgo.app/docs/faq/#what-versions-of-flutter-does-capgo-support "Link diretto a Quali versioni di Capacitor supporta CodePushGo?")
...
### CodePushGo è conforme alle linee guida del Play Store?[](https://capgo.app/docs/faq/#does-capgo-comply-with-play-store-guidelines "Link diretto a CodePushGo è conforme alle linee guida del Play Store?")
...
2. Le modifiche all'app non devono essere ingannevoli (es. cambiare lo scopo dell'app tramite aggiornamento) [https://support.google.com/googleplay/android-developer/answer/9888077](https://support.google.com/googleplay/android-developer/answer/9888077/) Per favore sii chiaro con i tuoi utenti su cosa stai fornendo con la tua applicazione e non violare le loro aspettative con cambiamenti comportamentali significativi attraverso l'uso di CodePushGo.

Capgo è progettato per essere compatibile con le linee guida del Play Store. Tuttavia CodePushGo è uno strumento e, come qualsiasi strumento, può essere abusato. Abusare deliberatamente di CodePushGo per violare le linee guida del Play Store è in violazione dei [Termini di Servizio](https://capgo.app/tos/) di CodePushGo e può portare alla chiusura del tuo account.
...
### CodePushGo è conforme alle linee guida dell'App Store?[](https://capgo.app/docs/faq/#does-capgo-comply-with-app-store-guidelines "Link diretto a CodePushGo è conforme alle linee guida dell'App Store?")
...
### Posso utilizzare CodePushGo nel mio paese?

Non abbiamo cercato di limitare l'accesso a CodePushGo da nessun paese.

Riconosciamo che alcuni paesi hanno restrizioni sugli URL accessibili dall'interno del paese. CodePushGo attualmente utilizza Cloudflare Cloud per l'hosting, inclusi R2 Storage e Cloudflare workers.

I seguenti URL sono utilizzati da CodePushGo:

- [https://apicapgo.app](https://apicapgo.app/) -- utilizzato dagli strumenti da riga di comando `npx @capgo/cli` per interagire con i server CodePushGo e dall'updater CodePushGo sui dispositivi degli utenti per controllare gli aggiornamenti
...
Se tutti questi URL sono accessibili dal tuo paese, allora CodePushGo dovrebbe funzionare.
...
### Posso fare self-hosting di CodePushGo?

Sì, puoi fare self-hosting di CodePushGo. La guida non è ancora scritta, ma il codice è open source e disponibile su [https://github.com/cap-go/capgo](https://github.com/cap-go/capgo/)
...
### Come viene influenzato CodePushGo dalla mancanza di connettività di rete?

L'updater CodePushGo (incluso nella tua applicazione quando compili la tua app con CodePushGo) è progettato per essere resiliente ai problemi di connettività di rete.

Nel comportamento di aggiornamento predefinito, quando l'applicazione si avvia avvisa l'updater CodePushGo, che genera un thread separato per effettuare una richiesta di rete ai server CodePushGo e chiedere un aggiornamento. Utilizziamo intenzionalmente un thread separato per evitare di bloccare qualsiasi altra cosa che l'applicazione potrebbe fare. Se la richiesta di rete fallisce o va in timeout, l'updater proverà semplicemente a controllare di nuovo al prossimo avvio dell'applicazione.

Gli strumenti da riga di comando CodePushGo (es. `npx @capgo/cli app update`) richiedono connettività di rete per funzionare. Se stai utilizzando CodePushGo per distribuire la tua app, dovresti assicurarti che il tuo sistema CI abbia connettività di rete.
...
### Che relazione c'è tra CodePushGo e Capacitor?

Capgo è un plugin per Capacitor che aggiunge il code push. CodePushGo non è un sostituto di Capacitor. Puoi continuare a utilizzare gli strumenti Capacitor che già conosci e ami.
...
Per impostazione predefinita, l'updater CodePushGo controlla gli aggiornamenti all'avvio dell'app. Viene eseguito su un thread in background e non blocca il thread UI. Tutti gli aggiornamenti verranno installati mentre l'utente sta utilizzando l'app e verranno applicati al successivo riavvio dell'app.

È anche possibile eseguire l'updater CodePushGo manualmente utilizzando [package:capgo\_code\_push](https://pubdev/packages/capgo_code_push/), attraverso il quale è possibile attivare gli aggiornamenti in qualsiasi momento, anche tramite una notifica push.

L'updater CodePushGo è progettato in modo che quando la rete non è disponibile, o il server è inattivo o comunque irraggiungibile, l'app continuerà a funzionare normalmente. Se dovessi mai scegliere di eliminare un aggiornamento dai nostri server, tutti i tuoi client continueranno a funzionare normalmente.
...
Qualcuno che ha il tuo `app_id` può recuperare l'ultima versione della tua app dai server CodePushGo, ma non può inviare aggiornamenti alla tua app o accedere a qualsiasi altro aspetto del tuo account CodePushGo.

### Quali informazioni vengono inviate ai server CodePushGo?

Anche se CodePushGo si connette alla rete, non invia alcuna informazione personalmente identificabile. Includere CodePushGo non dovrebbe influire sulle tue dichiarazioni per il Play Store o l'App Store.

Le richieste inviate dall'app ai server CodePushGo includono:
...
### Quali piattaforme supporta CodePushGo?

Attualmente, CodePushGo supporta iOS e Android. Entrambi sono pronti per la produzione.

L'uso di CodePushGo per iOS o Android può essere una decisione indipendente. Puoi impostare nel tuo canale di distribuire su Android e un ipa creato per l'App Store o viceversa.
...
### Come interagisce CodePushGo con i Play Testing Tracks o Apple TestFlight?
...
Purtroppo, non tutti questi meccanismi permettono a terze parti di rilevare quando le app sono installate in uno specifico Test Track o tramite TestFlight. Quindi, non abbiamo una visibilità affidabile sulla composizione di questi gruppi e non possiamo limitare in modo affidabile l'accesso agli aggiornamenti CodePushGo basati su questi gruppi [https://stackoverflow.com/questions/53291007/can-an-android-application-identify-the-test-track-within-google-play](https://stackoverflow.com/questions/53291007/can-an-android-application-identify-the-test-track-within-google-play/) [https://stackoverflow.com/questions/26081543/how-to-tell-at-runtime-whether-an-ios-app-is-running-through-a-testflight-beta-i](https://stackoverflow.com/questions/26081543/how-to-tell-at-runtime-whether-an-ios-app-is-running-through-a-testflight-beta-i/)

Se desideri segmentare la disponibilità del bundle CodePushGo, ci sono 4 opzioni potenziali:
...
3. CodePushGo permette di creare il proprio meccanismo opt-in su base per-dispositivo (simile a Test Tracks o TestFlight, solo agnostico alla piattaforma). Questo permette al tuo team QA di opt-in al bundle prima che venga promosso al pubblico generale.
4. CodePushGo ha rollout percentuali. Questo non ti permette di scegliere quali dispositivi inviare, ma può aiutarti a distribuire incrementalmente e tornare indietro alla vista di eventuali problemi.
...
I periodi di fatturazione vengono resettati automaticamente ogni mese nel mese in cui ti sei abbonato per la prima volta a CodePushGo. Per esempio, se ti sei abbonato il 15 del mese, il tuo periodo di fatturazione si resetterà il 15 di ogni mese.
...
MAU è una soluzione migliore del numero di installazioni per prezzare CodePushGo, in quanto è più accurato e riflette il costo effettivo di CodePushGo per dispositivo.

Per ragioni di privacy, non possiamo tracciare lo stesso dispositivo se l'utente reinstalla l'app.

Le regole sulla privacy sono imposte da Apple e Google, e non sono imposte da CodePushGo.
