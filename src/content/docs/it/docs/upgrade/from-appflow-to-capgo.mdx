---
title: Migrazione da AppFlow a CodePushGo
description: Guida completa per migrare la tua app da Ionic AppFlow a CodePushGo
sidebar:
  order: 7
locale: it
---
...
Questa configurazione ti aiuterà a mappare le funzionalità di AppFlow con gli equivalenti di CodePushGo.
...
```typescript
// Equivalente CodePushGo in capacitor.config.ts
{
  plugins: {
    CapacitorUpdater: {
      autoUpdate: true,
...
Se stai utilizzando la strategia "always latest" di AppFlow, implementa con CodePushGo:
...
| Metodo AppFlow | Equivalente CodePushGo | Note |
|----------------|------------------|-------|
...
```
## Perché migrare a CodePushGo?

Con l'annuncio della chiusura di Ionic AppFlow, la migrazione a CodePushGo offre una transizione perfetta per il tuo flusso di lavoro di sviluppo di app mobili. CodePushGo offre funzionalità migliorate, prestazioni superiori e significativi risparmi sui costi mantenendo tutte le funzionalità critiche necessarie.
...
Aggiungi la configurazione CodePushGo al tuo `capacitor.config.json`:
```json
{
  "plugins": {
    "CapacitorUpdater": {
...
1. Creare canali nella dashboard CodePushGo:
```bash
...
1. [Crea un account CodePushGo](https://capgo.app/register/)
